...
import { BoardColumnsSettingsSection } from './components';
...
export default function BoardColumnsSettings({
    board,
}: BoardSettingsPageProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Главная', href: '/dashboard' },
        { title: board.title, href: boardsRoute.show(board.id).url },
        { title: `Настройки колонок`, href: '#' },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Колонки доски ${board.title}`} />

            <BoardSettingsLayout board={board}>
                <BoardColumnsSettingsSection board={board} />
            </BoardSettingsLayout>
        </AppLayout>
    );
}
